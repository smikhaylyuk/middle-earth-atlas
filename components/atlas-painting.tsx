...
export const AtlasPainting=memo(forwardRef<AtlasPaintingHandle,{onLoad:()=>void;onError:()=>void}>(function AtlasPainting({onLoad,onError},ref){
  ...
  useEffect(()=>{
    ...
    image.src=assetPath('/images/atlas-painted.webp');
    return()=>{image.onload=null;image.onerror=null;bitmap.current=null;};
  },[draw,onLoad,onError]);
  return <><link rel="preload" as="image" href={assetPath('/images/atlas-painted.webp')}/><canvas ref={canvas} className="atlas-painting" aria-hidden="true"/></>;
}));
