...
const field=buildRiverField([track(layout.anduin),track(layout.entwash),...gondor.anduin.map(track),track(pelennor.anduin),track(morgul.morgulduin),...Object.values(lamedon.rivers).map(track)],water);
const {data:paint,info:paintInfo}=await sharp('public/images/atlas-lamedon.webp').ensureAlpha().raw().toBuffer({resolveWithObject:true});
