...
export function registerAtlasTools<TPlace extends PlaceId=PlaceId>(actions:{supportedPlaces?:readonly TPlace[];read:()=>AtlasState;focus:(id:TPlace)=>void;overview:()=>void;lighting:(mode:LightMode)=>void;route:(visible:boolean)=>void;motion?:(enabled:boolean)=>void;intensity?:(value:MotionIntensity)=>void;time?:(hour:number)=>void;cycle?:(enabled:boolean)=>void;duration?:(seconds:number)=>void;bree?:(detail:BreeDetail|null)=>void;region?:(region:RegionId)=>void}) {
  ...
  const tools:Tool[]=[{
    ...
    name:'configure_atlas_view',title:'Explore Middle-earth',description:'Explore western Middle-earth, from the Grey Havens and Rivendell through Eregion and Moria to Isengard, east across the mountains to Lórien and the Anduin valley, and south into Rohan as far as Edoras, Dunharrow and the Argonath, then through Nen Hithoel to Rauros and Nindalf, east across Emyn Muil to the Dead Marshes, Dagorlad and the Black Gate, then through Udûn and the Isenmouthe to Mount Doom and Barad-dûr, and down the Anduin to Cair Andros, Henneth Annûn, Drúadan Forest and Amon Dîn, continuing to Minas Tirith, Osgiliath, Harlond and Emyn Arnen, then east to the Cross-roads, Minas Morgul and Cirith Ungol, and west beneath the White Mountains to Erech, Tarlang’s Neck, Calembel and Ethring, then west to Drúwaith Iaur, the Lefnui, Pinnath Gelin and Anfalas, and south to Edhellond, Cobas Haven, Dol Amroth and Belfalas, then east to Linhir, the Gilrain–Serni junction and western Lebennin. region frames west, east, south, anduin, rohan, rauros, morannon, mordor, gondor, pelennor, morgul, lamedon, anfalas, belfalas, linhir or all; it cannot be combined with place or an open Bree detail. Explore a place and configure atmosphere. breeDetail opens the Prancing Pony detail view in Bree and selects a book-based point of interest; closed returns to the map. Opening a Bree detail cannot be combined with another place. Navigating to a place closes an open detail. routeVisible shows the illustrative road, not Frodo’s exact itinerary. The artwork is an interpretation. timeOfDay sets a 0–24 hour clock shared by the map and inn; cycleEnabled starts or holds it. cycleDuration is seconds per day. motionEnabled pauses all motion, including time. Lighting presets hold morning or evening light. These controls never advance historical journey dates.',
    inputSchema:{type:'object',properties:{place:{type:'string',enum:[...availablePlaces,'overview']},region:{type:'string',enum:['west','east','south','anduin','rohan','rauros','morannon','mordor','gondor','pelennor','morgul','lamedon','anfalas','belfalas','linhir','all']},breeDetail:{type:'string',enum:['prancing-pony','sign','courtyard','hobbit-rooms','closed']},lighting:{type:'string',enum:['morning','golden']},routeVisible:{type:'boolean'},motionEnabled:{type:'boolean'},motionIntensity:{type:'string',enum:['subtle','lively']},timeOfDay:{type:'number',minimum:0,exclusiveMaximum:24},cycleEnabled:{type:'boolean'},cycleDuration:{type:'number',enum:[30,60,120]}},additionalProperties:false,minProperties:1},annotations:{readOnlyHint:false,untrustedContentHint:false},
    async execute(input){
      ...
      if(v.region!==undefined&&(!['west','east','south','anduin','rohan','rauros','morannon','mordor','gondor','pelennor','morgul','lamedon','anfalas','belfalas','linhir','all'].includes(v.region as string)||!actions.region))throw new Error('Unknown map region.');
